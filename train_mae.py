...
class MAE(nn.Module):
    ...
    def forward(self, img):
        """
        Standard forward pass that computes the reconstruction MSE loss on the masked patches.
        """
        device = img.device
        ...
        unmasked_decoder_tokens = unmasked_decoder_tokens + self.decoder_pos_emb(unmasked_indices)
        ...
        pred_pixel_values = self.to_pixels(decoded_tokens[batch_range, masked_indices])
        loss = F.mse_loss(pred_pixel_values, masked_patches)
        return loss

    def reconstruct(self, img):
        """
        Given an input image, this method returns a full reconstructed image.
        (It follows the same masking procedure as forward().)
        """
        device = img.device
        patches = self.to_patch(img)
        tokens = self.patch_to_emb(patches)
        ...
        decoded_tokens = self.decoder(all_decoder_tokens)
        # only reconstruct the masked tokens as the ViT is not trained to
        # reconstruct the unmasked tokens during the computation of training loss
        decoded_tokens[batch_range, unmasked_indices] = self.patch_to_emb(patches)[batch_range, unmasked_indices]
        # Predict pixel values for ALL patches:
        full_pred = self.to_pixels(decoded_tokens)  # shape: (batch, num_patches, patch_dim)

        # Unpatchify: reshape tokens back into images.
        patch_height, patch_width = self.patch_size
        image_height, image_width = self.image_size
        ...
        recon_img = rearrange(
            ...
            h = num_patches_h, p1 = patch_height, w = num_patches_w, p2 = patch_width, c = self.channels
        )
...
class MAELightningModule(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if batch_idx == 0:
            recon = self.mae.reconstruct(imgs)
            # Make a grid (clamp to [0,1] for visualization)
            recon_grid = make_grid(recon.clamp(0,1), nrow=8)
            # self.logger.log_image(
            #     key="reconstructed_images",
            #     images=[wandb.Image(recon_grid, caption="Epoch {}".format(self.current_epoch))]
            # )
            self.logger.experiment.log({"reconstructed_images": [wandb.Image(recon_grid, caption="Epoch {}".format(self.current_epoch))]})
        return loss
...
# Main function
# ------------------------------
@hydra.main(config_path="configs", config_name="mae")
def main(cfg: DictConfig):
    ...
    model = MAELightningModule(argparse.Namespace(**hparams))
